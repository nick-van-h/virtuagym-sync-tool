...
$arr = [];
echo ('Select user ID for Testuser, then getOne');
br();
$sql = "SELECT `id`
        FROM `users`
        WHERE `username` = 'Testuser'";
foreach ($arr as $val) {
        echo ('Value in array: ' . $val);
        br();
}
$db->query($sql);
...
echo ('==> Expected outcome: [value_str] => admin');
br();
...
echo ('Update token to FooBarBaz');
br();
$sql = "UPDATE settings 
...
echo ('Query ran with result: ' . $db->getStatus());
br();
echo ('Update token to FooBarBaz123');
br();
$sql = "UPDATE settings 
        SET value_str=(?), value_int=(?), type=(?) 
        WHERE setting_name=(?)
...
echo ('<h2>Getters</h2>');
echo ('Password hash: ' . $settings->getPasswordHash());
br();
echo ('User role: ' . $settings->getRole());
br();
echo ('Key encrypted: ' . $settings->getKeyEnc());
br();
echo ('Username (enc): ' . $settings->getVirtuagymUsernameEnc());
br();
echo ('Password (enc): ' . $settings->getVirtuagymPasswordEnc());
br();
echo ('Username from token (CHBS): ' . $settings->getUsernameFromToken('CorrectHorseBatteryStaple'));
br();
echo ('Token expiry date: ' . $settings->getTokenExpiryDate());
br();

echo ('<h2>Update & restore</h2>');
$org = $settings->getRole();
echo ('Original role: ' . $org);
br();
$settings->setRole('dork');
echo ('Change role, result: ' . $settings->getRole());
br();
$settings->setRole($org);
echo ('Restore role, result: ' . $settings->getRole());
br();
